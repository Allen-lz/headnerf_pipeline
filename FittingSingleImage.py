...
class FittingImage(object):
    ...
    def load_data(self, img, mask_img, nl3dmm_para_dict):
    ...
    def fitting_single_images(self, img, mask_img, para_3dmm_dict, tar_code_path, save_root):
        
        self.load_data(img, mask_img, para_3dmm_dict)
...
if __name__ == "__main__":
    ...
    # 这个应该是人脸编辑的时候要的用的latent code, 如果只是重建3d人脸的话, 应该不用这个
    ...
    gpu_id = args.gpu_id
    # 得到mask模型
    mask_model = GenHeadMask(gpu_id=gpu_id)

    # 得到3dmm模型
    model_3dmm = FittingNL3DMM(img_size=args.img_size, intermediate_size=args.intermediate_size, gpu_id=gpu_id)
    ...
    bgr_img = cv2.resize(bgr_img, (args.img_size, args.img_size))

    mask_res = mask_model.main_process_image(bgr_img=bgr_img)

    para_3dmm_dict = model_3dmm.main_process_image(bgr_img=bgr_img, img_path=img_path)

    tt = FittingImage(model_path, save_root, gpu_id=0)
    tt.fitting_single_images(bgr_img,
                             mask_res,
                             para_3dmm_dict,
                             target_embedding_path,  # 这个是ref, 如果不是做人脸编辑的话, 可以不要的
                             save_root)
