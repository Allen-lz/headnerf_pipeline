...
from Fitting3DMM.NL3DMMRenderer import NL3DMMRenderer
...
from Fitting3DMM.DatasetforFitting3DMM import DatasetforFitting3DMM
from HeadNeRFOptions import BaseOptions
import argparse
from DataProcess.Gen_Landmark import Gen2DLandmarks


class FittingNL3DMM(object):
    def __init__(self, img_size, intermediate_size, gpu_id, batch_size=1, img_dir=None) -> None:
        ...
        # 得到关键点模型
        self.landmarks_model = Gen2DLandmarks()
    ...
    def opt_batch_data(self, w2c_Rmats, w2c_Tvecs, inmats, imgs, lms, base_names):
        ...
        save_res_data = self.save_res(iden_codes, expr_codes, text_codes, illu_codes, w2c_Rmats, w2c_Tvecs, inmats, base_names)
        return save_res_data
    ...
    def main_process(self):
        ...
        for id_s in loop_bar:
            ...
            # temp_data是一个dict类型, **是为了解包
            save_res_data = self.opt_batch_data(**temp_data)
            # 对batch中的每个数据进行保存
            # 这里保存的数据主要就是将, imagename和文件名一一对应起来
            for item in save_res_data:
                res = item[0]
                save_path = item[1]
                with open(save_path, "wb") as f:
                    pkl.dump(res, f)

    def main_process_image(self, bgr_img, img_path):
        """
        bgr_img: opencv直接读取图片路径得到的数据
        """
        # 得到landmark
        landmark = self.landmarks_model.main_process_image(bgr_img)

        # 将数据输入到接口中获得3dmm的输入参数
        temp_data = self.data_utils.load_one_image(bgr_img, landmark=landmark, img_path=img_path)
        # temp_data是一个dict类型, **是为了解包
        save_res_data = self.opt_batch_data(**temp_data)[0]
        # 对batch中的每个数据进行保存
        # 这里保存的数据主要就是将, imagename和文件名一一对应起来
        res = save_res_data
        return res
