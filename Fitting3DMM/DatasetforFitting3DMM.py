...
class DatasetforFitting3DMM(Dataset):
    ...
    def load_one_image(self, bgr_img, landmark, img_path):
        """
        bgr_img: 使用opencv读取图片路径得到的数据
        landmark: landmark模型的输出
        image_path: 待处理的图片的路径(这个可能不必要)
        """
        # 1. 图片名称预处理(这个应该可以不要)
