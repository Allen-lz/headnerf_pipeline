...
class FaceAlignment:
    def __init__(self, landmarks_type, network_size=NetworkSize.LARGE,
        ...
        face_detector_module = __import__('faceAlignment.face_alignment.detection.' + face_detector,
    ...
    def get_landmarks_from_image(self, image_or_path, detected_faces=None, return_bboxes=False,
        ...
        for i, d in enumerate(detected_faces):
            ...
            # 基于上面计算出来的系数对图片进行中心的crop + scale
            inp = crop(image, center, scale)
            # 用于可视化的输入图片
            vis_inp = inp.copy()
            vis_inps.append(vis_inp)
            ...
            # 这个结果是(68, 2)
            # preds应该是已经映射到了原图的大小上
            # pts_vis = np.array(pts.detach().cpu())
            # face = vis_inp
            # h, w, c = face.shape
            # img = np.zeros((h, w, c), dtype=np.uint8)
            # plt.title(str(i))
            # for index, p in enumerate(pts_vis):
            #     # cv2.circle(img, i.astype('uint8') - np.array([face_bbox[0], face_bbox[1]]), 1, (255, 255, 255), -1)
            #     cv2.circle(vis_inp, p.astype('uint8'), 2, (255, 0, 0), -1)
            #     cv2.circle(img, p.astype('uint8'), 2, (255, 255, 255), -1)
            #     # plt.imshow(np.concatenate([img, face], axis=1))
            # plt.imshow(np.concatenate([vis_inp, img], axis=1))
            # plt.show()
        ...
        if return_bboxes or return_landmark_score:
            return landmarks, landmarks_scores, vis_inps
        else:
            return landmarks
